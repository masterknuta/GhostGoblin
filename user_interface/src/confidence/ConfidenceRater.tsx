...
const ConfidenceRater: React.FC<ConfidenceRaterProps> = ({ onClose }) => {
  ...
  // Clear image data on component unmount or when closing
  useEffect(() => {
    return () => {
      clearSensitiveData();
    };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only accept images for privacy & security
    ...
    reader.onload = (e) => {
      ...
      if (data && data instanceof ArrayBuffer) {
        ...
        // If readAsDataURL fallback, convert to ArrayBuffer (optional)
    ...
    // Read as ArrayBuffer to avoid any cached blob URLs
    reader.readAsArrayBuffer(file);
  };

  // Simulate feedback with delay and humor mode selection
  const simulateFeedback = () => {
    ...
    setTimeout(() => {
      const feedbacks = humorMode ? humorFeedbacks : normalFeedbacks;
      setFeedback(feedbacks[Math.floor(Math.random() * feedbacks.length)]);
      setProcessing(false);
      // Immediately clear imageData for security
      setImageData(null);
    }, 2000);
  };
        ...
        <div className="flex-1 p-6 flex flex-col justify-center space-y-6">
          ...
          <div className="bg-dark-gray border border-glowing-violet rounded-lg p-4 space-y-4">
            ...
            <ul className="text-white text-sm space-y-2">
              ...
              <li>• Save or keep images beyond processing</li>
            </ul>
          </div>
  ...
  return (
    <div className="h-full bg-deep-purple flex flex-col">
      <div className="p-4 border-b border-glowing-violet flex items-center justify-between">
        ...
        <button onClick={() => { clearSensitiveData(); onClose(); }} className="p-1 hover:bg-glowing-violet rounded">
          <X className="w-5 h-5 text-white" />
        </button>
      </div>
      ...
      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        ...
        <div className="border-2 border-dashed border-glowing-violet rounded-lg p-8 text-center">
          ...
          <label
            htmlFor="file-upload"
            className="cursor-pointer px-6 py-2 bg-glowing-violet hover:bg-opacity-80 rounded-lg font-bold text-white transition-colors inline-block"
          >
            Choose Image
          </label>
        </div>
        ...
        {processing && (
          <p className="text-cyan text-center font-orbitron mt-2 animate-pulse">
            Processing your image...
          </p>
        )}

        {feedback && (
          <div className="bg-dark-gray border border-neon-green rounded-lg p-4 mt-4">
        ...
        <div className="bg-glowing-violet bg-opacity-10 border border-glowing-violet rounded-lg p-3 mt-6">
          <p className="text-glowing-violet text-xs text-center">
            This interface is designed for maximum privacy. No images are saved or shared. GG's personality is
            strictly non-retentive and cannot save your images.
          </p>
        </div>
      </div>
    </div>
